...
/// <summary>
/// Shared-state collection: tests that use static RealmHub calibration dictionaries
/// must not run in parallel with each other or with hub tests that do the same.
/// </summary>
[CollectionDefinition("CalibrationCleanup", DisableParallelization = true)]
public class CalibrationCleanupCollection { }
...
public class RealmCleanupServiceTests
{
    ...
    // -------------------------------------------------------------------------
    // AdminAuth token cleanup integration
    // -------------------------------------------------------------------------
    ...
    public async Task ExecuteAsync_CallsCleanupExpiredTokens()
    {
        ...
        var authConfig = new ConfigurationBuilder()
            ...
            .Build();
        var adminAuth = new AdminAuthService(authConfig);

        // Issue a token (it won't be expired but cleanup must not throw)
        adminAuth.Login("admin", "secret");

        var service = new FastCleanupService(manager, statsTracker, adminAuth, _loggerMock.Object);
        using var cts = new CancellationTokenSource();
...
// =============================================================================
// Calibration session cleanup — runs NON-PARALLEL to avoid static-dict races
// =============================================================================

/// <summary>
/// Tests for calibration session TTL cleanup in the RealmCleanupService.
/// These tests modify the shared static RealmHub calibration dictionaries and
/// are placed in a dedicated [Collection] to prevent parallel execution with
/// other tests that also create calibration sessions.
/// </summary>
[Collection("CalibrationCleanup")]
public class CalibrationSessionCleanupTests
{
    /// <summary>Testable subclass with an extra-short calibration TTL for testing.</summary>
    private class FastCalibrationCleanupService : RealmCleanupService
    {
        protected override TimeSpan CleanupInterval => TimeSpan.FromMilliseconds(50);
        protected override TimeSpan EndedRealmTtl => TimeSpan.FromMinutes(30);
        protected override TimeSpan AbandonedRealmTtl => TimeSpan.FromMinutes(15);
        protected override TimeSpan CalibrationSessionTtl => TimeSpan.FromMilliseconds(1);

        public FastCalibrationCleanupService(RealmManager realmManager, RealmStatsTracker statsTracker,
            AdminAuthService adminAuth, ILogger<RealmCleanupService> logger)
            : base(realmManager, statsTracker, adminAuth, logger) { }
    }
    ...
    private static AdminAuthService CreateAdminAuthService() =>
        new AdminAuthService(new ConfigurationBuilder().Build());

    [Fact]
    public async Task ExecuteAsync_CleansUpExpiredCalibrationSessions()
    {
        var manager = CreateRealmManager();
        var statsTracker = new RealmStatsTracker();
        var loggerMock = new Mock<ILogger<RealmCleanupService>>();

        // Use unique non-numeric IDs that won't be generated by CreateCalibrationSession
        var sessionId = "cleanup-test-session-" + Guid.NewGuid().ToString("N");
        var joinCode = "CLNUP-" + Guid.NewGuid().ToString("N");

        var session = new PulseRealm.Server.Hubs.RealmHub.CalibrationSession
        {
            SessionId = sessionId,
            JoinCode = joinCode,
            DashboardConnectionId = "dash-conn-id",
        };

        PulseRealm.Server.Hubs.RealmHub._calibrationSessions[sessionId] = session;
        PulseRealm.Server.Hubs.RealmHub._calibrationJoinCodes[joinCode] = sessionId;

        try
        {
            // Wait for the session to become "expired" (TTL is 1ms)
            await Task.Delay(20);

            var service = new FastCalibrationCleanupService(manager, statsTracker, CreateAdminAuthService(), loggerMock.Object);
            using var cts = new CancellationTokenSource();
            ...
            Assert.False(PulseRealm.Server.Hubs.RealmHub._calibrationSessions.ContainsKey(sessionId));
            Assert.False(PulseRealm.Server.Hubs.RealmHub._calibrationJoinCodes.ContainsKey(joinCode));
        }
        finally
        {
            // Ensure cleanup even if assertions fail
            PulseRealm.Server.Hubs.RealmHub._calibrationSessions.TryRemove(sessionId, out _);
            PulseRealm.Server.Hubs.RealmHub._calibrationJoinCodes.TryRemove(joinCode, out _);
        }
    }

    [Fact]
    public async Task ExecuteAsync_LogsExpiredCalibrationSessionCount()
    {
        var manager = CreateRealmManager();
        var statsTracker = new RealmStatsTracker();
        var loggerMock = new Mock<ILogger<RealmCleanupService>>();

        // Inject a stale calibration session with unique IDs
        var sessionId = "log-test-session-" + Guid.NewGuid().ToString("N");
        var joinCode = "LOGT-" + Guid.NewGuid().ToString("N");
        var session = new PulseRealm.Server.Hubs.RealmHub.CalibrationSession
        {
            SessionId = sessionId,
            JoinCode = joinCode,
            DashboardConnectionId = "dash-conn",
        };
        PulseRealm.Server.Hubs.RealmHub._calibrationSessions[sessionId] = session;
        PulseRealm.Server.Hubs.RealmHub._calibrationJoinCodes[joinCode] = sessionId;
        ...
        try
        {
            await Task.Delay(20); // wait for TTL to elapse
            ...
            loggerMock.Verify(
                l => l.Log(
                    ...
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((v, _) =>
                        v.ToString()!.Contains("calibration session")),
                    It.IsAny<Exception?>(),
                    It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
                Times.AtLeastOnce);
        }
