...
private fun PersonalSummaryPage(
    ...
    val elevation = personal?.elevationGainMeters ?: summary.elevationGainMeters
    ...
    ScalingLazyColumn(
        ...
        if (elevation > 0) {
            item {
                val elevText = if (elevation >= 1000) {
                    "%.2f km".format(elevation / 1000)
                } else {
                    "%.1f m".format(elevation)
                }
                StatItem(value = elevText, label = "ELEVATION", color = PulseColors.Amber)
            }
        }
...
private fun TeamSummaryPage(teamName: String?, teamMembers: List<ClientSummaryData>) {
    ...
    val teamElevation = teamMembers.sumOf { it.elevationGainMeters }
    ...
    ScalingLazyColumn(
        ...
        if (teamElevation > 0) {
            item {
                val elevText = if (teamElevation >= 1000) {
                    "%.2f km".format(teamElevation / 1000)
                } else {
                    "%.1f m".format(teamElevation)
                }
                StatItem(value = elevText, label = "ELEVATION", color = PulseColors.Amber)
            }
        }
...
private fun RealmSummaryPage(summary: RealmSummaryData) {
    ...
    ScalingLazyColumn(
        ...
        if (summary.elevationGainMeters > 0) {
            item {
                val elevText = if (summary.elevationGainMeters >= 1000) {
                    "%.2f km".format(summary.elevationGainMeters / 1000)
                } else {
                    "%.1f m".format(summary.elevationGainMeters)
                }
                StatItem(value = elevText, label = "ELEVATION", color = PulseColors.Amber)
            }
        }
