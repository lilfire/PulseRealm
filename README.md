...
A live treadmill session platform with wearable integration and multiple run modes.
...
│  Client     │  ◄── session commands ──── │   Server     │ ◄── actions ─── │   Dashboard │
└─────────────┘                            └──────────────┘                 └─────────────┘
```

- **Server** (`server/`) — ASP.NET Core 8 Web API with SignalR hub for real-time communication
- **Frontend** (`frontend/`) — React + TypeScript + Vite web dashboard for session management and live rendering
...
1. A user opens the **web dashboard** and creates a **session** by selecting a **mode**
2. The session generates a **short join code** (Kahoot-style, e.g. `ABC123`)
3. The **wearable client** enters the code and connects to the session
...
More modes can be added by extending the `SessionMode` type and creating a new React component in `frontend/src/components/modes/`.
...
The server starts at `http://localhost:5062` with the SignalR hub at `/hubs/session`.
