...
export function StreetViewLobby({ joinCode, mode, clients, clientProfiles, connected, onStart, onLeave, onEnd, onKick, role, hostSecret, curatedLocations, lobbySettings, onSettingsChange, onRequestBind, onCancelBind, bindCode, bindPending, bindResult, boundClientId, clientBindings, serverUrl, popularity }: Props) {
  ...
  const rawLocations = curatedLocations && curatedLocations.length > 0 ? curatedLocations : CURATED_LOCATIONS;
  const getLocationKey = (loc: StreetViewLocation) => `streetview:${loc.lat.toFixed(4)},${loc.lng.toFixed(4)}`;
  const locations = sortByPopularity
    ? [...rawLocations].sort((a, b) => ((popularity?.[getLocationKey(b)] ?? 0) - (popularity?.[getLocationKey(a)] ?? 0)))
    : rawLocations;
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", paddingTop: "1.5rem", textAlign: "left" }}>
        <div style={{ maxWidth: "420px", flexShrink: 0 }}>
            ...
            <div style={{ position: "relative", width: "100%" }}>
              <input
                ...
                style={{
                  ...
                  fontSize: "1rem",
                  borderRadius: "6px",
                  border: "2px solid " + (location ? "#33DFFF" : "#555"),
                  width: "100%",
                  background: "#1a1a1a",
                  color: "#fff",
          ...
          <div style={{ display: "flex", alignItems: "center", margin: "1rem 0 0.5rem" }}>
            <p style={{ color: "#aaa", fontSize: "0.85rem", margin: 0 }}>Or pick a location:</p>
            <button
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{loc.address}</div>
                  {(popularity?.[getLocationKey(loc)] ?? 0) > 0 && (
                    <div style={{ fontSize: "0.75rem", color: "#666", marginTop: "0.15rem" }}>{popularity![getLocationKey(loc)]}x played</div>
                  )}
                </div>
              </div>
            );
