...
function getDifficultyParams(difficulty: DungeonDifficulty, timeframe: number): DifficultyParams {
  ...
  // Step-based thresholds scale by timeframe only — difficulty does NOT affect
  // the amount of work, only how hard the mechanics are (cadence windows,
  // HR thresholds).  This keeps total duration close to the chosen timeframe
  // regardless of difficulty.
  const tf = timeframe / 30; // 15min=0.5, 30min=1, 45min=1.5, 60min=2

  const base = {
    // Step-based (scale with timeframe only)
    corridorTiles: Math.max(1, Math.round(2 * tf)),
    enemyHp: Math.round(250 * tf),
    trapSafeTarget: Math.round(120 * tf),
    trapDamageLimit: Math.round(80 * tf),
    bossHp: Math.round(500 * tf),
    bossTrapSafeTarget: Math.round(120 * tf),
    bossTrapDamageLimit: Math.round(80 * tf),

    // Time-based (scale with timeframe only)
    treasureDuration: Math.round(15 + tf * 5),
    restHoldSeconds: 30,
    bossEnduranceSeconds: 45,

    // Difficulty-only thresholds (affect challenge, not duration)
    restHrThreshold: 114,
