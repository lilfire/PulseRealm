...
export function AdminDashboard({ apiUrl, token, onLogout, onJoinRealm }: Props) {
  ...
  async function handleBackup() {
    try {
      ...
      a.download = match?.[1] ?? "pulserealm-backup.json";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch {
      setSaveMsg("Backup failed");
    }
  }
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setSaveMsg(err?.error ?? "Restore failed");
          return;
        }
        const data = await res.json();
        setConfig(data);
      ...
      <aside className={`admin-sidebar${sidebarOpen ? " admin-sidebar-open" : ""}`}>
        ...
        <div className="admin-sidebar-footer">
          <div className="admin-sidebar-footer-row">
            <button onClick={handleBackup} className="admin-btn-footer">Backup</button>
            <button onClick={handleRestore} className="admin-btn-footer">Restore</button>
          </div>
