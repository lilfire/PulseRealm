...
import { useRealmHub, type RealmSummary, type ConnectionStatus } from "./hooks/useSessionHub";
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/api/config`)
      ...
      .then((data) => {
        ...
        if (data.popularity && typeof data.popularity === "object") {
          setPopularity(data.popularity as Record<string, number>);
        }
      })
      .catch(() => {});
  }, [apiUrl, configRefresh]);
      ...
      <div className="home-screen">
        ...
        <footer className="server-footer">
          ...
          <div className="footer-right">
            <a className="btn-tos-link" href="https://github.com/lilfire/PulseRealm" target="_blank" rel="noopener noreferrer">
              GitHub
            </a>
  ...
  function withBanner(content: React.ReactNode) {
    return <>{<ConnectionBanner status={connectionStatus} />}{content}</>;
  }
  ...
  return withBanner(
    <div className="app">
      ...
      <p>Status: {statusLabels[connectionStatus]}</p>
    </div>
  );
}
